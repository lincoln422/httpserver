	...
	while(1)
	{
		...
		else
		{
			if(pool->mutex_data.worker_num < pool->max_workers)//add a new worker
			{
				...
				{
					pthread_mutex_unlock(&(pool->info_lock));
					sem_post(&(pool->surplus_task_num));   //原子操作加1
					free(idle_worker);
					continue;
				}
...
void * worker_routline(void * arg)
{
	...
	while(1)
	{
		...
		if(pool->flag == SHUTDOWN)//pool is shutting
		{
			...
			pthread_cond_destroy(&(this_worker->worker_cond));     //删除互斥锁和条件变量
...
void * worker_manage(void * arg)
{
	...
	while(1)
	{
		if(pool->flag == SHUTDOWN)//now the pool should be closed
		{
			...
			{
				worker_to_del = list_entry(pos, struct Gthread_pool_worker, link_node);  //类型是woeker
				del_worker(worker_to_del, pool);
				free(worker_to_del);
				pool->mutex_data.worker_num--;   //将所有的worker一一删除
			}
			
			list_for_each(pos, &(pool->task_list))//delete all tasks
			{
				task_to_del = list_entry(pos, struct Gthread_pool_task, link_node);  //类型变了 变成了 task
				list_del(pos);
				free(task_to_del);
				pool->mutex_data.task_num--;		//将池中任务链表的所有的task一一删除
			}
			pthread_mutex_unlock(&(pool->info_lock));
		...
		if(get_pool_usage(pool) < LODE_GATE)			//负载小于某个值的时候会减少worker数量
		{
			worker_to_del = search_idle_worker(pool);  //循环直到找到空闲的worker为止
			if(worker_to_del == NULL)
			{
				sleep(1);
				continue;
			}											//找到了，把他复制给workertodel，删除，释放内存
			pthread_mutex_lock(&(pool->info_lock));
			del_worker(worker_to_del, pool);
			free(worker_to_del);
			pool->mutex_data.worker_num--;
