...
*/
			...
			else if(events[i].events & EPOLLRDHUP)// a client close the fd
			{
				...
				del_event(epoll_fd, events[i].data.fd, DATA_IN);   //删除事件
				pthread_mutex_lock(&timer_lock);
...
{ 
	...
	int n;
	n = get_line(client_fd, rcv_buffer, BUFSIZ);  //获取一行的输入数据，last non-void charater is '\n'
#if DEBUG == 1
	printf("%s", rcv_buffer);
#endif
	int i = 0, j = 0;
	...
	method[i] = '\0';//此时i已经移动到最后一位了，所以最后一位改动成\0
	...
	{
		//can not under stand the request，同时get post？？
...
int get_line(int sock, char *buf, int size)
{
	...
	while ((i < size - 1) && (c != '\n'))  //还没识别到换行符或者i还没达到size就循环
	{
	n = recv(sock, &c, 1, 0);  //读进C里面
	if(n > 0)
	{
		if(c == '\r') //回车
		{
			n = recv(sock, &c, 1, MSG_PEEK);//MSG_PEEK,仅把tcp buffer中的数据读取到buf中,
...
void cat(int client, FILE *resource)
{
 ...
 while (!feof(resource))//一般在文件操作，中经常使用feof()判断文件是否结束。没有结束就循环继续
 {
  send(client, buf, strlen(buf), 0);  //一直获取文件里面的内容到buf里，在发送出去，实现“传文件”
  fgets(buf, sizeof(buf), resource);
 }
}
...
// int i;
 ...
 if (pid == 0)  /* child: CGI script */
 {
  ...
  putenv(meth_env); //改变或增加环境变量的内容。REQUEST_METHOD=method
  if (strcasecmp(method, "GET") == 0) {
   sprintf(query_env, "QUERY_STRING=%s", query_string);
   putenv(query_env);
  }
  else {   /* POST */  //若不是get 就是post？
